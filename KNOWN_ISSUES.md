...
- **H-5 FFI re-entry surface — miri-detected aliasing UB (deferred, 2026-05-10)**. The Phase 2 nightly miri workflow flagged Undefined Behaviour in `core/tests/handler_reentry_guard.rs` on its first scheduled run: every protected FFI entry (`bb_term_clear_all`, `bb_term_take_snapshot`, `bb_term_text_range`, `bb_term_resize`, `bb_term_resize2`, `bb_term_current_mode`) materialises `&mut bb.term` BEFORE checking the `FFI_HANDLER_IN_FLIGHT` thread-local latch. The latch correctly suppresses any aliased read or write, so the runtime behaviour is observably correct (and the same tests pass under regular `cargo test` on every PR), but the materialisation itself takes a unique tag on the borrow stack while the outer `bb_term_input`'s tag is still active — both Stacked Borrows and Tree Borrows correctly classify this as UB whether or not we go on to use it. **Restructure landed (code), miri re-validation still pending (2026-06-20).** The reorder the entry called for is now in place: `ffi_reentry_blocked` (a thread-local read, NO `*term` deref) runs at the top of EVERY guarded entry — `bb_term_input`, `bb_term_resize2`, `bb_term_take_snapshot`, `bb_term_current_mode`, `bb_term_scroll`, `bb_term_scroll_to_bottom`, `bb_term_clear_all`, `bb_term_text_range`, `bb_term_set_named_color`, … — BEFORE that entry materialises its `&mut *term` / `&*term`, so a re-entrant call bails before taking a second borrow (verified by reading `core/src/lib.rs`; the stale "checked only by bb_term_input" comment was corrected). What remains is the miri *confirmation* that the borrow stack is now clean under Tree Borrows so `handler_reentry_guard.rs` can drop its `#[cfg_attr(miri, ignore)]` — that needs a nightly-miri run (the toolchain isn't exercised in the normal PR gate), so it stays ignored until that run is done. The non-miri runs continue to pin the runtime invariants on every PR. v1.0 hardening item.
