...
|--------------------|-----------------|--------|---------------------------------------------------------------------|----------------------------------------------------------------------------------------|
| Claude Code        | latest CLI      | ✅     | TUI, paste, ⌘C copy, ⌘V paste, OSC 8 click, find (⌘F), large output | Daily driver — Blackbird's design target. Its REPL runs in the alt screen with DEC 1000/1002/1003/1006 mouse reporting on for the whole session; ⌘-click link opening works *through* that grab (issue #30). OSC 8 emission depends on `TERM` containing `kitty`, which Blackbird sets when the bundled kitty terminfo installs.                        |
