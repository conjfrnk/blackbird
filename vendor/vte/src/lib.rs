...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn action_osc_put(&mut self, byte: u8) {
        ...
        // [Blackbird vendored patch] Bound the std-mode `Vec<u8>` so an
        // unterminated OSC (opened with `ESC ]` and never closed by BEL / ST /
        // CAN / SUB) can't grow without limit — a memory DoS reachable from
        // ordinary hostile terminal output. Upstream only caps under
        // `#[cfg(not(feature = "std"))]` (the ArrayVec path); the `std` `Vec`
        // path had no cap at all. The cap is generous — far past any legitimate
        // OSC (OSC 52 clipboard / OSC 8 hyperlinks are KiB-scale) — so it only
        // ever truncates a pathological/hostile sequence.
        #[cfg(feature = "std")]
        {
            const STD_OSC_RAW_CAP: usize = 8 * 1024 * 1024; // 8 MiB
            if self.osc_raw.len() >= STD_OSC_RAW_CAP {
                return;
            }
        }
