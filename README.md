...
- **Clipboard.** ⌘C/⌘V with paste and copy scrubbing (strips C0 controls and Unicode bidi overrides). OSC 52 clipboard writes (Neovim, tmux `set-clipboard on`, `pbcopy`-style helpers over ssh) are off by default and enabled in Settings → Security; writes are capped and scrubbed, and reads are never answered. OSC 10/11/12 colour queries are answered (rate-capped) so TUIs can detect light/dark; Settings → Security turns that off.
...
- **Notifications.** Programs that emit OSC 9 (iTerm2 form), OSC 777 `;notify;` or kitty OSC 99 reach Notification Center when the tab isn't the one you're looking at, and the tab pill gets a dot until you return; a bell in a background tab bounces the Dock icon (Settings → Behavior: Bell = Visual / Sound / both / Off). Claude Code doesn't auto-detect Blackbird as a notification target yet — run `claude config set preferredNotifChannel terminal_bell` once (or `iterm2` for OSC 9) and it will.
