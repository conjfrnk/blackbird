    ...
    internal func reconcile(stored: [WeakWindow],
        ...
        for w in live where !seen.contains(ObjectIdentifier(w)) {
            if let hintIndex = departureHints.firstIndex(where: { $0.departed === w }) {
                ...
                // One-shot: consumed on the FIRST join anywhere, matched or
                // not. Keeping an unmatched hint alive would let a window
                // that tail-joined an unrelated group jump next to its old
                // neighbour on a later join — pinned by
                // TabOrderCoordinatorTests.
