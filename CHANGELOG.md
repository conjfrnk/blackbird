...
- Kitty keyboard protocol, flag 1 (disambiguate): plain Esc is `CSI 27 u`, and Ctrl+key / Alt+key / Shift+Alt+key are `CSI <key>;<mod>u` as the spec requires (only unmodified Enter, Tab and Backspace stay legacy). The encoder had kept bare `ESC`, `ESC a` and the C0 byte for letters — the very ambiguity the flag exists to remove — and its tests pinned the deviation. Legacy bytes still flow when no kitty flag is active.
...
- Tabs: right-clicking a background pill no longer switches to it before the menu appears; a departure hint (move a tab away, then back) survives an intermediate group's refresh; the `+` button's action is a typed selector; the private `NSBezierPath.cgPath` copy is gone (SDK 14 provides it).
